...
def visualize_segmentation(seg_arr, inp_img=None, n_classes=None,
                           ...
                           prediction_width=None, prediction_height=None, prob_seg_arr=False):
...
def predict(model=None, inp=None, out_fname=None,
            ...
            read_image_type=1,prob_seg_arr=False,
            resize = False):
    ...
    if prob_seg_arr:
        pr_multi = pr.reshape((output_height, output_width, n_classes))
        if resize:
            #pr = cv2.resize(pr, (inp.shape[1], inp.shape[0]), interpolation=cv2.INTER_NEAREST)
            print(f"shape of pr is {pr.shape}")
            
        foreground_proba_multi = np.maximum(pr_multi[:,:,1], pr_multi[:,:,2])
        return foreground_proba_multi
    ...
    if out_fname is not None:
        in_h = cv2.resize(inp, (output_width, output_height))
        seg_img_resized = cv2.resize(seg_img, (in_h.shape[1], in_h.shape[0]))

        out_img = np.hstack((in_h, seg_img_resized))
