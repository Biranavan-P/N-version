...
def train(model,
    ...
    print(f"loss_k: {loss_k}")
    print(f"model: {model}")
    from .models.all_models import model_from_name
    # check if user gives model name instead of the model object
    if num_gpu > 1 and not isinstance(model, six.string_types):
        raise ValueError("Cannot train model on multiple GPUs if the model is built outside of this function.")
    if isinstance(model, six.string_types):
        ...
        if (input_height is not None) and (input_width is not None):
            ...
            if num_gpu > 1:
                ...
                with strategy.scope():
                    strategy_metrics = get_metrics_multi_class(n_classes=n_classes)
                    
                    
                    
                    
                    
                    if len(metrics) != len(strategy_metrics):
                        raise ValueError("Length of metrics should be the same.")
                 ...
                 model = model_from_name[model](
                n_classes, input_height=input_height, input_width=input_width)
            print("created model " + str(model.model_name))
